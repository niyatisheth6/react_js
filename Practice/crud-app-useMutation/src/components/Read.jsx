...
function Read() {
  const { data, refetch: refetchGetAll } = useQuery(["resKey"], () => {
    return axios
      .get("https://64a3a3c9c3b509573b565930.mockapi.io/crud")
      .then((res) => res?.data);
  });
  const { mutate: mutateDelteFn } = useMutation((value) => deleteData(value), {
    onSuccess: () => {
      refetchGetAll();
    },
  });
  ...
  function deleteData(id) {
    return axios.delete(
      `https://64a3a3c9c3b509573b565930.mockapi.io/crud/${id}`
    );
  }
  function handleEdit(id, name, email, age) {
